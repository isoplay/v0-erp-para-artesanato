...
export function CalendarioContent({ pedidos }: { pedidos: PedidoCalendario[] }) {
  ...
  function getPedidosForDate(date: Date) {
    return pedidos.filter((p) => {
      ...
      return isSameDay(parseISO(p.prazo_entrega), date)
    })
  }
  ...
  const hoje = new Date()
  const entregasHoje = pedidos.filter((p) => p.prazo_entrega && isSameDay(parseISO(p.prazo_entrega), hoje))
  const entregasAtrasadas = pedidos.filter(
    (p) => p.prazo_entrega && parseISO(p.prazo_entrega) < hoje && !isSameDay(parseISO(p.prazo_entrega), hoje)
  )
